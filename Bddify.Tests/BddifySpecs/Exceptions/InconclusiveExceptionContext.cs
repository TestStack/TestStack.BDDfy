...
namespace Bddify.Tests.BddifySpecs.Exceptions
{
    public class InconclusiveExceptionContext
    {
        ...
        InconclusiveTestClass TestClass
        {
            get
            {
                return (InconclusiveTestClass)_bddifier.Scenario.Object;
            }
        }
        ...
        ExecutionStep GivenStep
        {
            get
            {
                return _bddifier.Scenario.Steps.First(s => s.Method == Helpers.GetMethodInfo(TestClass.GivenAClassUnderTest));
            }
        }
        ...
        ExecutionStep WhenStep
        {
            get
            {
                return _bddifier.Scenario.Steps.First(s => s.Method == Helpers.GetMethodInfo(TestClass.WhenInconclusiveExceptionIsThrownInOneOfTheMethods));
            }
        }
        ...
        ExecutionStep ThenStep
        {
            get
            {
                return _bddifier.Scenario.Steps.First(s => s.Method == Helpers.GetMethodInfo(TestClass.ThenTheContextIsFlaggedAsInconclusive));
            }
        }
        ...
        {
            Assert.That(_bddifier.Scenario.Result, Is.EqualTo(StepExecutionResult.Inconclusive));
        }
